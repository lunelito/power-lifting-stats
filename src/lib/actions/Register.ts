...
export async function register(
  ...
  try {
    ...
    await supabase.auth.admin.inviteUserByEmail(result.data.email, {
    ...
    if (error) {
      return {
        errors: {
          _form: [error?.message || "Failed to register user"],
        },
      };
    } else {
      return {
        success: true,
        errors: {
          _form: ["Account created, verify your email on " + result.data.email],
        },
      };
    }
  } catch (err: unknown) {
